...
/**
 * Floating "speak with us" button. Opens WhatsApp once `contact.whatsapp` is
 * set in content/site.js (pre-filled with the property on a listing page);
 * until then it offers a phone call, on small screens only.
 */
export default function FloatingContact() {
  const match = useMatch('/properties/:slug')
  const [phone] = contactLinks.phones

  if (contactLinks.whatsapp) {
    const property = match && getProperty(match.params.slug)
    const message = property
      ? `Hello Giwagate, I’m interested in the ${property.title.toLowerCase()} in ${property.location}.`
      : 'Hello Giwagate, I’d like to speak with you about a property.'
    ...
    return (
      ...
      >
        <svg width="24" height="24" viewBox="0 0 24 24" aria-hidden="true">
          <path fill="currentColor" d={whatsappIcon} />
        </svg>
  ...
  return (
    <a className="floating-contact floating-contact--call" href={phone.href} aria-label={`Call us on ${phone.display}`}>
      <svg width="22" height="22" viewBox="0 0 24 24" aria-hidden="true">
        <path fill="currentColor" d={phoneIcon} />
      </svg>
      <span className="floating-contact__label">Call us</span>
    </a>
  )
}
